...
for i=1:n
    ...
    if CollisionFree(PointOfRand,PointOfNearest)==1 %ObtacleFree==CollisionFree
        ...
        V0fXnear = Near(V,PointOfNew);
        for kk=1:size(V0fXnear,1)
            if CollisionFree(V(V0fXnear(kk),:),PointOfNew)==1
                Flag_valid=1+Flag_valid;
                S(Flag_valid,:)=V0fXnear(kk);
                T(Flag_valid,:)=size(V,1);%IndexPointOfNew;
                E(Flag_valid,:)=pdist([V(V0fXnear(kk),1:2);PointOfNew(1:2)]);
            end
        end
    end
...
% for jj = 1 : size(P,2)-1
        ...
        plot(V(P(1,:),1),V(P(1,:),2),'r','linewidth', 2);
% end